...
// Функція для обчислення суми додатніх елементів
int sumPositiveElements(const std::vector<int>& array, int index) {
    if (index >= array.size()) {
        return 0;
    }
    return (array[index] > 0 ? array[index] : 0) + sumPositiveElements(array, index + 1);
}
...
int main() {
    ...
    // Виведення суми додатніх елементів
    int positive_sum = sumPositiveElements(array, 0);
    std::cout << "Сума додатніх елементів: " << positive_sum << std::endl;
